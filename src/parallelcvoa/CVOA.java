    ...
    protected void propagateDisease() {
        ...
        for (Individual x : infected) {
            ...
            if (x.getFitness() < bestSolution.getFitness()) {
                bestSolution = x;
                // Uncomment if you do not want to miss bestSolution when cancelling an execution 
        ...
        // Step 6. Add the current infected individuals to the recovered list (shared by all threads).
        recovered.addAll(infected);

        // Step 7. Update the infected list with the new infected individuals (shared by all threads).		
